...
bam_paths = list.files("", pattern = "", full.names = T)
bam_paths
```
...
design_data = data.frame( sample = c(),
                          group = c()
)
...
```{r write}
down_peaks %>% write_tsv("", col_names = F)
up_peaks %>% write_tsv("", col_names = F)
